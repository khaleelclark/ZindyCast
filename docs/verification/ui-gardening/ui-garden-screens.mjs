...
try {
 ...
 for(const width of [390,1440]){
  ...
  for(const tab of ['Today','Maps','History','Compare','Settings']){
   await page.getByRole('tab',{name:tab,exact:true}).click();await page.waitForTimeout(300);
   assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false,`${tab} overflow ${width}`);
   const controls=await page.locator('button:visible:not(.interactive-map button):not(.wbgt-marker):not([role=tab]),select:visible,input:visible:not([type=checkbox]):not([type=range])').evaluateAll(els=>els.map(el=>({text:el.getAttribute('aria-label')||el.textContent||el.id,height:el.getBoundingClientRect().height,font:getComputedStyle(el).fontSize})).filter(x=>x.height<43));
   assert.deepEqual(controls,[],`${tab} small controls ${width}`);
   await page.screenshot({path:`/tmp/ui-garden-${tab}-${width}.png`});results.push({tab,width});
  }
 }
 await page.getByRole('tab',{name:'Today',exact:true}).click();
 const input=page.locator('#city-search');await input.focus();await input.press('Tab');
 assert.equal(await page.evaluate(()=>getComputedStyle(document.activeElement).outlineStyle),'solid');
 assert.deepEqual(errors,[]);console.log(JSON.stringify({results,errors,outside}));
