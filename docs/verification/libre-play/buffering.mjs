...
async function setup({mode='ok',mobile=false}={}) {
 ...
 await c.route('**/*',async r=>{
  ...
  if(u.pathname.startsWith('/api/v1/maps/libre/tiles/')){
   calls.tiles.push(u.pathname);if(current==='slow') await new Promise(r=>setTimeout(r,200));const time=Number(u.pathname.split('/')[6]);
...
try {
 const a=await setup();await a.choose();await expect(a.p.getByText('Selected frame loaded.',{exact:true})).toBeVisible();await a.p.waitForTimeout(300);
 const before=await a.p.locator('.maplibregl-canvas').screenshot(); const label=await a.p.getByRole('slider',{name:'LibreWXR frames'}).locator('..').textContent();
 a.mode('slow');await a.p.getByRole('button',{name:'Play LibreWXR'}).click();await expect(a.p.getByRole('status').filter({hasText:'Buffering LibreWXR playback'})).toBeVisible();await a.p.waitForTimeout(250);
 expect(await a.p.getByRole('slider',{name:'LibreWXR frames'}).locator('..').textContent()).toBe(label);
 const buffered=await a.p.locator('.maplibregl-canvas').screenshot();expect(buffered.equals(before)).toBe(true);
 await a.p.getByRole('button',{name:'Pause LibreWXR'}).click();await expect(a.p.getByText('Selected frame loaded.',{exact:true})).toBeVisible();
 await a.p.getByRole('button',{name:'Play LibreWXR'}).click();await a.p.getByRole('slider',{name:'LibreWXR frames'}).focus();await a.p.getByRole('slider',{name:'LibreWXR frames'}).press('End');await expect(a.p.getByRole('button',{name:'Play LibreWXR'})).toBeVisible();
 await a.p.getByRole('button',{name:'Play LibreWXR'}).click();await a.p.getByRole('button',{name:'Refresh LibreWXR'}).click();await expect(a.p.getByRole('button',{name:'Play LibreWXR'})).toBeVisible();
 expect(a.calls.errors).toEqual([]);await writeFile(out+'/buffering.json',JSON.stringify({passed:true,previousCompleteCanvasPixelIdentical:true,previousLabelRetained:true,manualAndRefreshPause:true}));await a.c.close();
