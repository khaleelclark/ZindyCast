...
const dist=process.env.RADAR_CONTROLS_DIST || '/tmp/zindycast-flashing-dist';
...
try {
 ...
 for (const [width,height] of [[1440,1000],[390,844]]) {
  ...
  await page.locator('.interactive-map').scrollIntoViewIfNeeded();
  await page.screenshot({path:root+'/docs/verification/radar-flashing/today-'+width+'.png'});
 }
 assert.deepEqual(errors,[]);
 await writeFile(root+'/docs/verification/radar-flashing/today.json',JSON.stringify({passed:true,requests:requests.length,errors,viewports:['1440x1000','390x844'],chartHoverSamples:20,parentClockAdvanceMs:61000,stableCanvas:true,noReadyTransitions:true,limitations:'Synthetic weather timing and retained map fixtures; all external requests blocked. No physical device or live-weather claim.'},null,2));
