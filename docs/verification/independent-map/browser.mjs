...
try {
 ...
 for (const [width,height] of [[1440,1000],[1100,900],[390,844]]) {
  ...
  const mapBefore=await page.locator('.interactive-map').boundingBox();
  for(let i=0;i<4;i++) {
   await page.getByText('Heat precautions',{exact:true}).click();await page.waitForTimeout(600);
   const mapAfter=await page.locator('.interactive-map').boundingBox();
   assert.equal(mapAfter.width,mapBefore.width,'current disclosure must not resize map width');
   assert.equal(mapAfter.height,mapBefore.height,'current disclosure must not resize map height');
  }
  await page.getByRole('button',{name:'°C',exact:true}).click();await page.waitForTimeout(600);
  assert.equal((await page.locator('.interactive-map').boundingBox()).height,mapBefore.height,'unit change must not resize map');
  ...
  await page.screenshot({path:root+'/docs/verification/independent-map/width-'+width+'.png'});
 }
 assert.deepEqual(errors,[]);
 await writeFile(root+'/docs/verification/independent-map/results.json',JSON.stringify({passed:true,requests:requests.length,errors,viewports:['1440x1000','1100x900','390x844'],disclosureTogglesPerViewport:4,unitsChanged:true,mapDimensionsUnchanged:true,chartHoverSamples:20,parentClockAdvanceMs:61000,stableCanvas:true,noReadyTransitions:true,limitations:'Synthetic weather timing and retained map fixtures; all external requests blocked. No physical device or live-weather claim.'},null,2));
 console.log('PASS disclosure expansion/collapse, units, chart hover and clock do not resize/reload radar; same canvas, no readiness changes');
