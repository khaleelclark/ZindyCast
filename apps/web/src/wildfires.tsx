...
function visibleLimitation(text: string) {
  return !/no matching published records does not mean|disappearance from the current feed does not establish/i.test(text);
}
...
function WildfireSelection({ location, online, now }: { location: Location | null; online: boolean; now: number }) {
  ...
  return <Card component="section" className="wildfire-panel" aria-labelledby="wildfire-heading"><p className="eyebrow">NIFC / WFIGS · Published incident mapping</p><h2 id="wildfire-heading">Wildfire & prescribed-fire evidence</h2>
    <p className="notice">Perimeters are not evacuation zones or safety boundaries.</p><details><summary>Wildfire limitations</summary><p>Perimeters are not evacuation zones or spread predictions. Coverage and accuracy are not guaranteed.</p></details>
