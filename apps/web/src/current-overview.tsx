...
export function CurrentOverview({ forecast, observed, units, zone, now, fresh, busy, stationBusy, stationError, stationCheckedAt, refresh }: {
  ...
  return <div className="current-overview">
    <div className="current-summary" data-current-source={observed ? 'station' : 'model'}>
      ...
      <p className="current-time subtle">{observed ? <>Nearby station · {time(observed.observation.time)} · {Math.floor((now - Date.parse(observed.observation.time)) / 60_000)} min ago</> : <>{!modeled ? 'Current conditions unavailable' : fresh ? 'Model estimate' : 'Previously retrieved model estimate'}{modeled && <> · {time(modeled.time)}</>}</>} <Button size="small" disabled={busy || stationBusy} onClick={refresh}>{stationBusy ? 'Refreshing…' : 'Refresh'}</Button></p>
